...
**四种使用模式**：
- **模式 A**：完全独立（不用 Token Bank）
- **模式 B**：仅用网关（成本追踪）
- **模式 C**：仅用资源（MCP/Skill），直连官方 API
- **模式 D**：全部使用（完整功能）

**详细说明**：参见 `docs/modular-architecture.md`
...
**3. 兼容性**
- ✅ 用户在 Claude Desktop 的操作 → 可选：通过网关（成本追踪）或直连官方
- ✅ 用户在 Debug 页面的操作 → 可选：通过网关或直连
- ✅ 资源独立：即使不用网关，仍可使用 MCP/Skill 资源

**4. 模块化**
- ✅ **网关代理**是可选的（可直连官方 API）
- ✅ **资源管理**是独立的（MCP/Skill 可单独使用）
- ✅ 用户可以只用资源，不用网关
- ✅ 灵活组合，按需使用

**使用场景对比**：

| 场景 | 方式 A：Agent 直接使用 | 方式 B：通过网关 | 方式 C：直连 + TB 资源 | 方式 D：聚合入口 |
|-----|---------------------|---------------|---------------------|---------------|
| **配置** | 官方 API | Token Bank 网关 | 官方 API + TB MCP/Skill | Token Bank 网关 |
| **日常开发** | Claude Desktop | ✅ 可用 | ✅ 可用 | ✅ 可用 |
| **代码编辑** | Cursor | ✅ 可用 | ✅ 可用 | ✅ 可用 |
| **MCP 工具** | ❌ 需自己配置 | ✅ TB 统一管理 | ✅ TB 统一管理 | ✅ TB 统一管理 |
| **Skill 资源** | ❌ 需自己管理 | ✅ TB 投射 | ✅ TB 投射 | ✅ TB 投射 |
| **成本追踪** | ❌ 无 | ✅ 自动记录 | ❌ 无（直连官方） | ✅ 实时显示 |
| **统一界面** | ❌ | ❌ | ❌ | ✅ |

**说明**：
- **方式 A**：完全独立，不使用 Token Bank 任何功能
- **方式 B**：通过 Token Bank 网关，获得成本追踪
- **方式 C**：直连官方 API（不走网关），但使用 Token Bank 的 MCP/Skill 资源
- **方式 D**：通过 Debug 页面统一入口调用
