...
**Token Bank 是新一代个人AI资源管理中枢**，通过本地网关架构实现对 Claude、Cursor、Codex、WorkBuddy 等主流 AI 工具的一键纳管与智能调度。产品核心是**用量透明化**与**成本优化**：全链路 Trace 让每一笔 Token 消耗有据可查，智能路由自动在本地模型、免费额度、付费订阅与社区共享算力间无损切换，协议适配层支持 Anthropic Messages、OpenAI Chat、Codex Responses 的透明转换，让 Agent 无感使用第三方模型。技术上采用**声明式应用 Handler**（CLI 环境变量注入 + 配置文件热补丁）实现零侵入接入，通过 **MCP 内置中转**与**资源投射门控**构建个人知识与工具生态，并借助 **P2P 算力共享网络**将闲置资源变现为积分。**统一资产层架构**让社区智能体无需重复构建 harness，可直接运行在客户既有的 agent 应用（Codex、Claude、Cursor 等）上，在用户沉淀的 MCP/Skill/Prompt 资产中执行，实现运行时与工具生态的双重复用。**场景路由引擎**支持转发策略学习、上下文无损压缩，并可为非多模态模型自动注入图片识别能力（视觉增强层）。系统通过**基于使用习惯的进化机制**，从真实调用记录与会话模式中自动提炼工作画像，驱动 MCP/Skill/Prompt/Agent 的个性化推荐与路由策略持续优化，实现多设备用量云端聚合、Agent 聚合编排等能力，让 Token Bank 成为真正意义上的**越用越懂你的 AI 管家**。
