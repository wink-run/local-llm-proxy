...
> 用的明白 · 用的节省 · 闲置的还可以赚钱
>
> 一键纳管 Claude Code / Codex · CLI 多账号 · Agent 聚合编排 · MCP / Skill / Prompt 资源中枢 · 无损换模 · 全链路 Trace · 智能路由 · 社区 P2P
...
| 能力 | 说明 |
|---|---|
| **常见 Agent 纳管与 Trace** | 在网关页登记 Cursor、Claude Code、Codex CLI、Cherry Studio 等；走网关实时代理或会话补录，统一追溯每次调用 |
| **CLI 多账号** | 自动扫描 / 手工添加 Claude · Codex 等多实例；按启动目录分发账号；订阅额度与用量分账号统计 |
| **Agent 聚合编排** | 游乐场设主 Agent，聚合入口统一接任务，再协调派发至其他智能体；工具流与停止续接 |
| **资源中枢** | 社区推荐 MCP / Skill / Prompt / Agent 目录下发；按智能体投射门控；Prompt 经 `tokenbank-prompts` MCP 下发 |
| **个人订阅与盘点** | APP / API / 按量分栏管理；盘点按 **应用 · 供给源 · 模型 · 供给类型 · 费用 · 设备 · 时间** 切片；多设备云端聚合 |
