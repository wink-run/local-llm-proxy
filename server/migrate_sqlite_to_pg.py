...
用法（本地）:
  export DATABASE_URL=postgresql://root:changeme@localhost:5432/tokenbank
  python migrate_sqlite_to_pg.py --sqlite ./proxy.db

用法（docker compose 已启动 postgres + proxy）:
  docker compose exec -e DATABASE_URL=postgresql://root:changeme@postgres:5432/tokenbank \\
...
挂载旧库示例:
  docker compose run --rm -v $(pwd)/server/proxy.db:/backup/proxy.db \\
    -e DATABASE_URL=postgresql://root:changeme@postgres:5432/tokenbank \\
    proxy python migrate_sqlite_to_pg.py --sqlite /backup/proxy.db --truncate
"""
