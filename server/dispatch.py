...
async def handle_image(body: dict, consumer_user_id: int | None = None):
    """Dispatch an image generation request to a worker (always non-streaming)."""
    model = body.get("model", "")
    ...
    try:
        # Send with endpoint hint so workers and virtual workers route correctly
        await worker.send({"type": "request", "req_id": req_id, "payload": body, "endpoint": "images"})
    except Exception:
        worker.pending.pop(req_id, None)
        worker.active_requests = max(0, worker.active_requests - 1)
        raise HTTPException(502, "Failed to reach worker")
