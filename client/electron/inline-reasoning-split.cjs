...
function isTruncatedThinking(text) {
...
function joinThinkingOutput(thinking, output) {
 ...
 */
function findUserFacingCjkIndex(text) {
 ...
 */
function findUserFacingEnIndex(text) {
  const raw = String(text || '');
  if (!raw) return -1;
 ...
 */
function hasClearReasoningBoundary(text) {
...
module.exports = { normalizeLoose, looksLikeInlineReasoning, looksLikeLeakedReasoning, hasReasoningMeta, findUserFacingStart, findUserFacingCjkIndex, findUserFacingEnIndex, hasClearReasoningBoundary, isTruncatedThinking, joinThinkingOutput, dedupeRepeatedText, stripDuplicateThinkingPrefix, stripReasoningLeakage, repairThinkingOutputBoundary, sanitizeThinkingOutputPairs, splitInlineReasoning, expandMixedOutputSteps, dedupeConsecutiveSteps };
