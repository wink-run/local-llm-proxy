...
function composeHandoffDoc({ brief, project, sourceAgent } = {}) {
  return [
    ...
    `> 来源：${sourceAgent || '?'} 会话（该会话已结束；下文写给完全没有上下文的接手者）`,
    '',
    brief || '',
    '',
    '---',
    `以上是此前在 ${sourceAgent || '另一个 agent'} 上的完整工作交接。`,
    '你没有此前会话的上下文。请先用一两句话确认你对任务、卡点与下一步的理解，然后直接继续推进，不要向用户重新索要背景。',
  ].join('\n');
}
...
const HANDOFF_SYSTEM = [
  '你是“会话交接”助手。当前会话即将结束。',
  '请根据下面的会话记录（含原始目标、最近进展、可选 git 状态），写一份中文交接文档。',
  '读者是一个完全没有上下文的新会话 / 新 agent：用户会把文档整段粘贴过去，对方必须仅凭此文档就能继续工作。',
  '写清楚、写具体、可执行；不要空话，不要寒暄，不要解释你在做什么。',
  '信息不足时基于上下文合理推断，并在该处标注「(推断)」。',
  '',
  '必须按以下 markdown 小节输出（顺序固定，不要增删标题）：',
  '## 任务 — 我们在做什么：用户最终想达成什么、怎样算完成（从原始请求提炼，勿只复述最近动作）',
  '## 已完成 — 已经做完并可信的事项：关键改动、结论、已验证结果（具体到文件/行为）',
  '## 当前卡点 — 卡在哪、卡住的原因、已尝试过什么、现状（无卡点则写「无」并说明进行到哪一步）',
  '## 下一步计划 — 接手者应立即执行的有序步骤（可直接照做，带必要命令/路径/验收点）',
  '## 绝对不要再踩的坑 — 本会话踩过或发现的坑、错误假设、无效路径、用户明确否定过的做法（没有则写「无」）',
  '## 关键文件与决策 — 相关文件及其作用；重要约定、约束、用户偏好',
  '',
  '只输出交接文档本身。',
].join('\n');
