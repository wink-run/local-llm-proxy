    ...
    <div className="p-6 space-y-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3">
          ...
          <div className="text-[10px] text-zinc-400 mt-0.5">{t('network.onlineWorkers')}</div>
        </div>
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3">
          ...
          <div className="text-[10px] text-zinc-400 mt-0.5">{t('network.dedupModels')}</div>
        </div>
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3">
          ...
          <div className="text-[10px] text-zinc-400 mt-0.5">{t('network.thisPeriod')}</div>
        </div>
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3">
          ...
          <div className="text-[10px] text-zinc-400 mt-0.5">{t('network.contributing')}</div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden">
            <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
              ...
              <span className="text-[10px] text-zinc-400">{t('network.sortByNodes')}</span>
            </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {size && (
                          <span className="text-[9px] text-zinc-400 bg-zinc-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded">{size}</span>
                        )}
                      </div>
                      {m.nodes > 0 && !avgS && (
                        <div className="text-[10px] text-green-600 mt-0.5">{t('network.idle')}</div>
                      )}
                    </div>
                    <div className="text-right shrink-0">
                      {m.nodes > 0 ? (
                        <>
                          ...
                          <div className="text-[10px] mt-0.5 text-zinc-400">
                        ...
                        <>
                          ...
                          <div className="text-[10px] text-zinc-400">{t('network.unavailable')}</div>
                        </>
                      )}
                    </div>
                  </div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden">
              <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
                ...
                <span className="text-[10px] text-zinc-400">{t('network.leaderboardHint')}</span>
              </div>
              <div className="divide-y divide-gray-200/50 dark:divide-gray-800/50">
                {topWorkers.length === 0 ? (
                  ...
                  return (
                    <div key={w.worker_id || w.name} className="flex items-center gap-3 px-5 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {modelSummary && (
                          <div className="text-[10px] text-zinc-400 mt-0.5 truncate">{modelSummary}</div>
                        )}
                      </div>
                      <div className="text-right shrink-0">
                        ...
                        <div className="text-[10px] text-zinc-400">{w.avg_latency_ms ?? 0} ms</div>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                {myStats && myStats.active_workers > 0 ? (
                  ...
                  <span className="text-[10px] bg-zinc-100 dark:bg-zinc-800 text-zinc-400 border border-zinc-700 dark:border-zinc-700 px-1.5 py-0.5 rounded-full">
                    {t('network.offline')}
                  </span>
                )}
              </div>
              {myStats ? (
                <>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-zinc-800/50 dark:bg-zinc-800/50 rounded-lg p-3">
                      <div className="text-[10px] text-zinc-400">{t('network.activeNodes')}</div>
                      <div className="text-lg font-bold mt-0.5 text-zinc-900 dark:text-zinc-100">{myStats.active_workers ?? 0}</div>
                    </div>
                    <div className="bg-zinc-800/50 dark:bg-zinc-800/50 rounded-lg p-3">
                      <div className="text-[10px] text-zinc-400">{t('network.activeRequests')}</div>
                      <div className="text-lg font-bold mt-0.5 text-zinc-900 dark:text-zinc-100">{myStats.active_requests ?? 0}</div>
                    </div>
                    <div className="bg-zinc-800/50 dark:bg-zinc-800/50 rounded-lg p-3 col-span-2">
                      <div className="text-[10px] text-zinc-400">{t('network.contribRate')}</div>
                  ...
                  <button onClick={() => navigate('/contribute')}
                    className="flex items-center justify-center gap-2 w-full py-2 text-xs text-blue-600 dark:text-blue-400 hover:text-blue-300 border border-zinc-700 dark:border-zinc-700 hover:border-zinc-700 dark:hover:border-zinc-700 rounded-lg transition-colors">
            ...
            {network.workers.length > 0 && (
              <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden">
                <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-zinc-400">{t('network.workersCount', { n: network.workers.length })}</span>
                </div>
