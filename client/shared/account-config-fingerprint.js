// 供给源账户配置指纹：MD5(规范化 JSON)，凭证只参与哈希不上报
// Node CLI 用此 CJS 副本；浏览器用 src/lib/account-config-fingerprint.js（保持逻辑一致）
'use strict';
...
function fingerprintSubscription(sub) {
  const payload = {
    t: 'sub',
...
function fingerprintDirect(agentId, billing) {
  const b = billing || {};
  const payload = {
    t: 'direct',
    agent_id: agentId || '',
...
function dedupeByConfigFp(items) {
  const map = new Map();
